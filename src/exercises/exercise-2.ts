...
validateDiscountCode(request.body).map((x) =>{
  ...
  .match(
    discount => {
      ...
      const discountedCart = mapDiscountedCart(request, discount);
...
function mapDiscountedCart(request: AuthenticatedRequest, discount: number) {
